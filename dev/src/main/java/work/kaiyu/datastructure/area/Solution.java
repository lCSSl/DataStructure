package work.kaiyu.datastructure.area;
...
public class Solution {
    ...
    public static int maxArea(int h, int w, int[] horizontalCuts, int[] verticalCuts) {
        ...
        int horizontalLowerBoundary = h - horizontalCuts[horizontalCuts.length - 1];
        maxH = maxH >= horizontalLowerBoundary ? maxH : horizontalLowerBoundary;
        for (int i = 1; i < verticalCuts.length; i++) {
            maxV = Math.max(maxV, verticalCuts[i] - verticalCuts[i - 1]);
        }
        int verticalLowerBoundary = w - verticalCuts[verticalCuts.length - 1];
        maxV = maxV >= verticalLowerBoundary ? maxV : verticalLowerBoundary;
